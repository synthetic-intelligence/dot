...
__powerline() {
    ...
    __git_info() { 
        ...
        local ref=$(timeout 1 $git_eng symbolic-ref --short HEAD 2>/dev/null)
        ...
        if [[ -n "$ref" ]]; then
            ...
            ref=$(timeout 1 $git_eng describe --tags --always 2>/dev/null)
        fi
        ...
        done < <(timeout 1 $git_eng status --porcelain --branch 2>/dev/null)  # note the space between the two <
